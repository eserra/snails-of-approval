...
export default function SnailDetail({ snail }: { snail: SnailData }) {
  ...
  const diversitySlugs = parseDiversityTags(
    snail.diversityTags as string | null
  );
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <DetailSection
        title="Info"
        snailId={snail.id}
        editForm={({ onSave, onCancel, saving }) => {
            ...
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    value={f.chapterId}
                    onChange={(e) => setF({ ...f, chapterId: e.target.value })}
                    ...
                    {diversityTagConfig.map((tag) => {
                      ...
                      return (
                        <label key={tag.slug} className="flex items-center gap-1.5 text-sm text-gray-700">
                          <input
                            type="checkbox"
                            checked={sel.includes(tag.slug)}
                            onChange={(e) => {
                              const next = e.target.checked
                                ? [...sel, tag.slug]
                                : sel.filter((s) => s !== tag.slug);
                              setF({ ...f, diversityTags: serializeDiversityTags(next) || "" });
                            }}
      ...
      >
        <dl className="grid gap-3 sm:grid-cols-2">
          ...
          <Field
            ...
            value={
              snail.category
                ? `${snail.category.parent?.name || ""} > ${snail.category.name}`.replace(/^ > /, "")
                : null
            }
          />
          <Field
            ...
            value={
              <span className="text-gray-400">
                {(snail.establishmentType as string) || "—"}
              </span>
            }
          />
          {diversitySlugs.length > 0 && (
            <Field
              label="Diversity / Ownership"
              value={diversitySlugs.map(getDiversityLabel).join(", ")}
            />
          )}
          {(snail.description as string) ? (
            <div className="sm:col-span-2">
              <Field label="Description" value={snail.description as string} />
            </div>
          ) : null}
        </dl>
      </DetailSection>
            ...
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select value={f.track} onChange={(e) => setF({ ...f, track: e.target.value, stage: e.target.value === "lead" ? "New" : "Onboarding" })} className={`${inputClass} bg-white`}>
                ...
                <div>
                  ...
                  <select value={f.stage} onChange={(e) => setF({ ...f, stage: e.target.value })} className={`${inputClass} bg-white`}>
                    {(f.track === "lead" ? leadStages : activeStages).map((s) => (
                      <option key={s} value={s}>{s}</option>
                    ))}
                  </select>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={f.formerAwardee} onChange={(e) => setF({ ...f, formerAwardee: e.target.checked })} className={checkboxClass} id="fa-edit" />
                  <label htmlFor="fa-edit" className="text-sm text-gray-700">Former Awardee</label>
                </div>
                <div>
                  <label className={labelClass}>Business Status</label>
                  <select value={f.businessStatus} onChange={(e) => setF({ ...f, businessStatus: e.target.value })} className={`${inputClass} bg-white`}>
                    <option value="">Select...</option>
                ...
                {f.formerAwardee && (
                  <div>
                    ...
                    <input type="number" value={f.yearAwarded} onChange={(e) => setF({ ...f, yearAwarded: e.target.value })} className={inputClass} />
                  </div>
                )}
                <div>
                  <label className={labelClass}>Renewal Due Year</label>
                  <input type="number" value={f.renewalDueYear} onChange={(e) => setF({ ...f, renewalDueYear: e.target.value })} className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Source</label>
                  <input value={f.source} onChange={(e) => setF({ ...f, source: e.target.value })} className={inputClass} />
                </div>
                {f.stage === "Blocked" && (
                  <div>
                    <label className={labelClass}>Blocked Reason</label>
                    <input value={f.blockedReason} onChange={(e) => setF({ ...f, blockedReason: e.target.value })} className={inputClass} />
                  </div>
                )}
              </div>
              <SaveCancel onSave={() => onSave(f)} onCancel={onCancel} saving={saving} />
      ...
      >
        <dl className="grid gap-3 sm:grid-cols-2">
          <Field
            label="Track"
            value={
              <span className="capitalize">{snail.track}</span>
            }
          />
          <Field label="Stage" value={snail.stage} />
          {snail.formerAwardee && (
            <>
              <Field label="Former Awardee" value="Yes" />
              {snail.yearAwarded && (
                <Field label="Year (First) Awarded" value={String(snail.yearAwarded)} />
              )}
            </>
          )}
      ...
      >
        <dl className="grid gap-3 sm:grid-cols-2">
          ...
          <Field
            ...
            value={
              snail.email ? (
                <a href={`mailto:${snail.email}`} className="text-amber-700 hover:text-amber-800">
                  {snail.email as string}
                </a>
              ) : null
            }
          />
          ...
          <Field
            ...
            value={
              snail.website ? (
                <a href={snail.website as string} target="_blank" rel="noopener noreferrer" className="text-amber-700 hover:text-amber-800 truncate block">
                  {(snail.website as string).replace(/^https?:\/\//, "")}
                </a>
              ) : null
            }
          />
                ...
                <div className="sm:col-span-2">
                  ...
                  <AddressAutocomplete
                    ...
                    onChange={(addr, lat, lon) => {
                      setF({ ...f, address: addr, ...(lat && lon ? { latitude: lat, longitude: lon } : {}) });
                    }}
                    className={inputClass}
      ...
      >
        <dl className="grid gap-3 sm:grid-cols-2">
          ...
          {snail.latitude ? (
            <Field label="Coordinates" value={`${snail.latitude}, ${snail.longitude}`} />
          ) : null}
        </dl>
      </DetailSection>
            ...
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select value={f.status} onChange={(e) => setF({ ...f, status: e.target.value })} className={`${inputClass} bg-white`}>
                    <option value="draft">Draft (hidden)</option>
                    <option value="published">Published (visible)</option>
                  </select>
                </div>
                <div className="flex items-center gap-2 self-end pb-2">
                  <input type="checkbox" checked={f.onSfusaMap} onChange={(e) => setF({ ...f, onSfusaMap: e.target.checked })} className={checkboxClass} id="sfusa-edit" />
                  <label htmlFor="sfusa-edit" className="text-sm text-gray-700">On SFUSA Map</label>
                </div>
              </div>
              <SaveCancel onSave={() => onSave(f)} onCancel={onCancel} saving={saving} />
            </div>
      ...
      >
        <dl className="grid gap-3 sm:grid-cols-2">
          <Field
            label="SFNYC Map"
            value={
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${snail.status === "published" ? "bg-green-50 text-green-700 ring-1 ring-green-600/20" : "bg-gray-100 text-gray-600"}`}>
                {snail.status === "published" ? "Published" : "Draft"}
              </span>
            }
          />
            ...
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select value={f.assigneeId} onChange={(e) => setF({ ...f, assigneeId: e.target.value })} className={`${inputClass} bg-white`}>
                ...
                <div>
                  ...
                  <input type="date" value={f.lastTouchDate} onChange={(e) => setF({ ...f, lastTouchDate: e.target.value })} className={inputClass} />
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={f.welcomeLetterSent} onChange={(e) => setF({ ...f, welcomeLetterSent: e.target.checked })} className={checkboxClass} id="wl-edit" />
                  <label htmlFor="wl-edit" className="text-sm text-gray-700">Welcome Letter Sent</label>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={f.stickersDelivered} onChange={(e) => setF({ ...f, stickersDelivered: e.target.checked })} className={checkboxClass} id="sd-edit" />
                  <label htmlFor="sd-edit" className="text-sm text-gray-700">Stickers Delivered</label>
                </div>
              </div>
              <SaveCancel onSave={() => onSave(f)} onCancel={onCancel} saving={saving} />
            </div>
      ...
      >
        <dl className="grid gap-3 sm:grid-cols-2">
          ...
          <Field
            label="Last Touch"
            value={snail.lastTouchDate ? new Date(snail.lastTouchDate as string).toLocaleDateString() : null}
          />
      ...
      {/* Attachments (always interactive) */}
